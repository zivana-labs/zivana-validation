...
test("RevenueFactDatum encodes to the exact CBOR of a real published Preprod transaction", () => {
  ...
  // exact inline-datum CBOR Blockfrost returned for a real Preprod
  // publish (see README's "Verification" section for the current tx
  // hash), built from the same known inputs used at the time. A
  // field-order regression here would change the byte sequence and fail
